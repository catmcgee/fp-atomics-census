...
The census pins vLLM at `98dff2a81d74`, FlashInfer at `8bc3b5780277` and
SGLang at `0bcd822377da`, the commits their v0.29.0, v0.6.18.post1 and
v0.5.19 tags point to; the wheels the probes below run are vLLM 0.28.0,
FlashInfer 0.6.16 and SGLang 0.5.19, so the FlashInfer wheel is older
than the pinned release. Line numbers are at the pinned shas.
...
directory argument. Comparisons can also be INVALID, NOT COMPARABLE or NOT TESTED. E3 does not test P2 because it neither reconstructs a recorded schedule nor teacher-forces a continuation. E6 is the P2 experiment: it records admissions and per-pass token ids, replays the recorded schedule in a fresh process with teacher forcing and compares every pass. The first eleven records and their same-stack replays came from one H100 software stack with vLLM 0.28.0 (in the third campaign one installed package, openai, differs), in three campaigns: Qwen2.5-7B-Instruct in a plain arm and a chunked-prefill arm; then Qwen3-8B-FP8 (blockwise FP8, compiled), Llama 3.1 8B Instruct (compiled) and Qwen1.5-MoE-A2.7B-Chat (uncompiled) in a plain arm each; then Qwen2.5-7B-Instruct at TP=2 in both arms, Qwen1.5-MoE-A2.7B-Chat uncompiled with CUDA graphs, Qwen3-8B-FP8 in a chunked-prefill arm, and Qwen2.5-7B-Instruct with online FP8 at per-token and at forced per-tensor dynamic activation scales. All eleven of those replays were IDENTICAL in every row of every pass, each run once in a new process on the same pod with the compiled artefacts its record had left on disk. Replayed again with every compile and kernel cache cleared, on a different pod with different GPUs of the same model and driver, and for the two Qwen2.5-7B TP=1 arms in a different datacentre and so certainly on a different machine, six of the eleven stayed IDENTICAL and five did not: the four compiled FP8 arms and the two uncompiled arms held, and every compiled bf16 arm differs from pass 0, slot 0, in 493 to 555 of its rows, with every comparison requirement met and every token held by teacher forcing. P2 therefore has supporting evidence, for these recorded schedules on this stack with vLLM 0.28.0, that is conditional on the compiled artefacts: it holds warm for all eleven, and cold for the FP8 and uncompiled arms only. Those eleven observations supply no evidence for other schedules, stacks or parallel configurations. Two further bounds come from the same campaign. Two cold replays of one record on one machine differed in 5 of 512 decode rows, so bit-identity here is an observation about the runs made rather than a demonstrated property. At TP=2 the comparison reads rank 0, and one cold replay has 4 of 555 decode rows where the two ranks' hidden-state hashes differ while their full-logit hashes and argmax agree, so a TP above 1 verdict describes rank 0. Those 0.28.0 records lack record-side artefacts and do not separate changed reduction configurations from alternative explanations. The later C1 evidence below supplies a bounded association on a different runtime. The forced per-tensor arm shows that a whole-batch replay reproduced bits that depend on every row of the pass through one activation scale. X2 predicts that such bits depend on the neighbours' content, and E6 re-supplies that content, so the result is consistent with X2 and is not a test of it; its per-tensor operation is inferred from the records, not recorded. A boundary rebuild of one decode pass from a single prefill of the recorded prefixes was DIFFERS with the argmax unchanged; the RUNBOOK explains why that mode cannot attribute a difference. E5 does not isolate GPU from driver or input divergence. A teacher-forced E6 replay of the H100 plain record on an RTX PRO 6000 met every comparison requirement and differed in every row; it is consistent with P5 but does not isolate the GPU, because the FlashAttention version, the driver, the container image and the compiled artefacts also differed.

[C1, on vLLM 0.29.0](../probes/shape/campaigns/2026-09-15-h100-e6-attribution.md), adds sixteen cold replays of one compiled bf16 record across two H100 machines: ten IDENTICAL with equal archived reduction choices and six DIFFERS with unequal choices. Four reduction signatures correspond one to one with four complete row-hash digests across twenty reference-arm runs, and restoring the original caches reproduces the record once on each machine. P2 evidence for this arm therefore depends on more than the recorded configuration; preserve compiled artefacts and report a rate when compiling afresh. A separate arm reproduces six differing decode positions in four replays, including the earlier five-row residual, but missing earlier record artefacts prevent direct attribution to a kernel across versions. C1's four online FP8 cold replays were IDENTICAL; this neither establishes FP8 immunity nor settles whether quantisation absorbs upstream differences. These are C1 results only. Its coarse compile observables, lack of launched-kernel profiling and absence of a single-choice intervention prevent a claim that reduction block size is the sole cause. The model, schedule, GPU model, TP=1 and runtime bounds remain; later campaigns require separate evaluation.


[C2 on the same 0.29.0 runtime](../probes/shape/campaigns/2026-09-15-h100-e6-modern.md) adds cold observations across ten configurations, including FP8 differences: per-token FP8 matched 1/3 and blockwise FP8 2/3. The older FP8/bf16 split is therefore specific to its samples. The combined deterministic-Inductor/combo-off control matched 3/3, but so did its plain baseline; no failing mixed-schedule control establishes a remedy. The batch-invariant control differed once and lacks two repeats. The MoE routing-count smoke check failed, although E6 schedule, forcing and output requirements passed; this is not evidence about routing counts. Neither the failed diagnostic nor the incomplete matrix is treated as a passed deployment configuration. MoE remained uncompiled on V1 and does not retest the earlier compiled/V2 defect.

The compiled MoE E4 arms are excluded from P4 evidence: a hook-free check on the same stack showed that the compiled configuration turns Qwen1.5-MoE-A2.7B-Chat's output degenerate on vLLM 0.28.0 even without CUDA graphs or the hook and with vLLM's compile cache disabled (Inductor's cache was not independently cleared), so their DIFFERS verdicts describe a mis-compiled model, while the uncompiled MoE arms, which are not degenerate, were IDENTICAL under neighbour replacement on that one stack.
