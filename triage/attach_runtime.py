...
ROWS: list[tuple[str, list[str], str]] = [
    ...
    (r"^sglang_qwen2.5_1.5b_gptq_marlin_stock_x6$", ["sglang-0008"], "SGLang 1.5B GPTQ on the stock 0.5.19 wheel (dead guard, atomic K-slice reduction live), model revision 4f5a0e31 pinned, same pod and wheel as the guard arms; 12 of 12 comparisons differ and the two processes start from different baseline hashes"),
    (r"^sglang_qwen2.5_1.5b_gptq_marlin_guard_off_x6$", ["sglang-0008"], "same-wheel control: the installed marlin_utils.py guard made live (probes/sglang_marlin_ablation/wheel_guard.patch) with SGLANG_MARLIN_USE_ATOMIC_ADD unset, so the atomic reduction is off; otherwise identical to the stock arm; both processes start from the same baseline hash; 4 of 12 comparisons differ in process with same-length observations and are not attributed to this row"),
    (r"^sglang_qwen2.5_1.5b_gptq_marlin_guard_env_on_x6$", ["sglang-0008"], "edited wheel with SGLANG_MARLIN_USE_ATOMIC_ADD=1 in the engine environment, re-enabling the atomic reduction; the variable is not among the flags the report records and is attested by the pod's /proc environ log in probes/sglang_marlin_ablation/; 12 of 12 comparisons differ and the two processes start from different baseline hashes, as on the stock wheel"),
    (r"^sglang_qwen2.5_1.5b_gptq_marlin_guard_off_det_x6$", ["sglang-0008"], "same-wheel control with the atomic reduction off and --enable-deterministic-inference; both processes start from the same baseline hash; 6 of 12 comparisons differ in process with same-length observations and are not attributed to this row"),
    (r"^sglang_qwen2.5_1.5b_gptq_marlin_guard_off_fixed_composition_x6$", ["sglang-0008"], "same edited wheel as guard_off (atomic reduction off, SGLANG_MARLIN_USE_ATOMIC_ADD unset) with max_running_requests=1, on another pod, so the engine runs one request at a time (configured; per-pass batch sizes are not recorded); both processes gave one observation in 14 evaluations, where guard_off had three distinct observations; one request at a time fixes batch composition but also changes every pass's row count and limits decode graphs to batch size 1, so this fits a batching effect and does not isolate composition as the cause of guard_off's residual differences"),
    ...
    # A rule mapping vllm_mixtral_gptq_moe_wna16_x6 to vllm-0014 was withdrawn: no such report exists, and on vLLM
    # 0.27.0 and later the moe_wna16 method uses TritonWNA16Experts and does not reach that CUDA kernel, so an
    # engine-level run on such a release would be not_reached rather than operator_example. Add a rule conditioned
    ...
    (r"^moe_wna16_cuda_", ["vllm-0014"], "moe_wna16 CUDA kernel through fused_experts on random 4-bit weights: 16 tokens, top-k 2, 8 experts (M * top_k / E = 4 <= 6 selects it). Run on the vLLM 0.28.0 wheel, not the pinned sha; the kernel file and should_moe_wna16_use_cuda are byte-identical at both."),
