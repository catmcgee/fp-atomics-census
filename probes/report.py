...
ROWS = [
    ...
    (r"^marlin_gemm_atomicTrue", "sglang-0008, vllm-0010 with atomic add on"),
    (r"^moe_wna16_cuda", "vllm-0014 (moe_wna16 CUDA kernel)"),
    (r"^moe_wna16_triton", "vllm-0014 method, Triton path (control)"),
    (r"^fi_b12x_moe", "flashinfer-0006..0008, flashinfer-0010 (SM120 CuTe DSL MoE finalize)"),
    (r"^batch_composition_", "scheduler instrumentation: composition signature per repeat (see the JSON)"),
    (r"^marlin_gemm_atomicFalse", "vllm-0010 (fp32 global reduce)"),
    (r"^vllm_.*tp2_nccl", "vllm-0182 (NCCL all-reduce at TP=2)"),
    (r"^vllm_.*tp2_custom", "vLLM custom all-reduce at TP=2"),
    (r"^vllm_mixtral.*moe_wna16", "vllm-0014 (moe_wna16 CUDA kernel)"),
    (r"^vllm_.*flashinfer_cutlass", "flashinfer-0001 through vLLM's flashinfer_cutlass MoE backend"),
    (r"^sglang_.*trtllm_mha", "sglang-0267 (TensorRT-LLM attention cubins)"),
