...
# kernel path: /root/mode-matrix-2/mode-3_graphs-off_runner-v2/cache/torchinductor/33/c33g6ouzhzjivox4thsamgwl5umzwnb6cjanzkv7h5564g553g4t.py
...
# Graph fragment:
#   %arg1_1 : Tensor "bf16[1, s27][s27, 1]cuda:0" = PlaceHolder[target=arg1_1]
#   %slice_1 : Tensor "bf16[1, (s27//2)][s27, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.slice.Tensor](args = (%arg1_1, 1, 0, %floordiv), kwargs = {})
...
#   %convert_element_type_1 : Tensor "bf16[1, (s27//2)][Max(1, (s27//2)), 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%div, torch.bfloat16), kwargs = {})
#   %slice_2 : Tensor "bf16[1, s27 - ((s27//2))][s27, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.slice.Tensor](args = (%arg1_1, 1, %floordiv, 9223372036854775807), kwargs = {})
#   %mul_6 : Tensor "bf16[1, (s27//2)][Max(1, (s27//2)), 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_1, %slice_2), kwargs = {})
#   return %mul_6
...
def call(args):
    ...
    args.clear()
    s27 = arg0_1
    assert_size_stride(arg1_1, (1, s27), (s27, 1), 'input')
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        arg1_1 = copy_if_misaligned(arg1_1)
        buf0 = empty_strided_cuda((1, s27 // 2), (max(1, s27 // 2), 1), torch.bfloat16)
        # Topologically Sorted Source Nodes: [getitem_2, silu, getitem_3, mul], Original ATen: [aten.slice, aten.silu, aten.mul]
        triton_poi_fused_mul_silu_slice_0_xnumel = s27 // 2
        raw_stream0 = get_raw_stream(0)
        triton_poi_fused_mul_silu_slice_0.run(arg1_1, buf0, s27, triton_poi_fused_mul_silu_slice_0_xnumel, stream=raw_stream0)
        del arg1_1
    return (buf0, )
...
def get_args():
    ...
    arg0_1 = 11264
    arg1_1 = rand_strided((1, 11264), (11264, 1), device='cuda:0', dtype=torch.bfloat16)
