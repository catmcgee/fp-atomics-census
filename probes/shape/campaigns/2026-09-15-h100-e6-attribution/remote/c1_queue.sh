#!/usr/bin/env bash
# C1 queue. Usage: c1_queue.sh DEADLINE_EPOCH JOBFILE
...
#   REST  record: run_e6.py --record arguments after the common ones
...
while IFS='|' read -r KIND LABEL CACHE ENVS REST; do
  ...
  if [ "$KIND" = "replay" ] || [ "$KIND" = "boundary" ]; then
    set -- $REST; WARM=$1
    for i in $(seq 1 60); do [ -f "$ROOT/$WARM/record/artefacts.json" ] && break; sleep 10; done
  ...
  case "$KIND" in
    record)
      eval "env $ENVS timeout 900 python run_e6.py --record --model $MODEL --revision $REV --out $ROOT --cudagraph 1 $REST" > "$LOGS/$LABEL.run.log" 2>&1 || rc=$? ;;
    replay)
      set -- $REST; ARM=$1; OUT=$2
      eval "env $ENVS timeout 900 python run_e6.py --replay $ROOT/$ARM/record --out $ROOT/$ARM/$OUT" > "$LOGS/$LABEL.run.log" 2>&1 || rc=$? ;;
    boundary)
      set -- $REST; ARM=$1; OUT=$2; P=$3
      eval "env $ENVS timeout 900 python run_e6.py --replay $ROOT/$ARM/record --out $ROOT/$ARM/$OUT --boundary $P" > "$LOGS/$LABEL.run.log" 2>&1 || rc=$? ;;
  esac
  ...
  grep -hE "Compiling a graph|torch.compile took|init engine|Graph capturing finished|FlashAttention|Selected |KV cache size|^E6 " $L | cut -c1-240 | sed "s/^/  [$LABEL] /"
