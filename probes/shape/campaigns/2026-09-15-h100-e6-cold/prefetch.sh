#!/usr/bin/env bash
# Prefetch the four pinned model revisions (small separate env, parallel to the venv build), then hash every file.
# HF_TOKEN arrives on stdin into the environment of the launching shell; it is never written to a file or echoed.
# Log: /root/logs/prefetch.log; digests: /root/logs/weights_digests.json
set -x
...
repos = (("Qwen/Qwen2.5-7B-Instruct", "a09a35458c702b33eeacc393d103063234e8bc28"),
         ("Qwen/Qwen3-8B-FP8", "220b46e3b2180893580a4454f21f22d3ebb187d3"),
         ("NousResearch/Meta-Llama-3.1-8B-Instruct", "d10aef7999a2b5ba950ab3974312feeedbfe0b77"),
         ("Qwen/Qwen1.5-MoE-A2.7B-Chat", "ec052fda178e241c7c443468d2fa1db6618996be"))
def get(rr):
    repo, rev = rr
...
list(ThreadPoolExecutor(2).map(get, repos))
PY
date -u
...
def h(j):
    ...
    return {"repo": repo, "revision": rev, "file": rel, "size": size, "sha256": s256.hexdigest(), "git_blob_sha1": s1.hexdigest(), "blob_path": os.path.basename(real)}
...
PY
du -sh /root/hf/hub/models--* 2>/dev/null
df -h /root | tail -1
date -u
echo "PREFETCH_DONE"
