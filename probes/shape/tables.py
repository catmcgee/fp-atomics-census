...
def execution_label(arm: str) -> str:
    if "_v2_" in arm:
        return ("compile off" if "_nocompile_" in arm else "compile on") + (", graphs off" if "_graphs0_" in arm else ", graphs requested")
    return "legacy eager (compile/graphs off)" if "_eager_" in arm or "_graphs0_" in arm else "legacy defaults (compile+graphs)"
